...
CREATE TEMP TABLE base_orders AS (
    SELECT DISTINCT
        ...
        o.order_datetime::date as order_date,  -- Convert to date directly
        o.shipped_units,
        ...
        v.company_code,
        v.company_name,
        'Pet' as gl_product_group_name  -- Simplified since you're only using 199
    ...
    WHERE v.company_code = 'BO92F'
        ...
        AND o.order_datetime::date BETWEEN CURRENT_DATE - interval '30 days' AND CURRENT_DATE
...
CREATE TEMP TABLE daily_sns_metrics AS (
    ...
    WHERE marketplace_id = 7
        AND gl_product_group =199
        AND TO_DATE(snapshot_date, 'YYYY-MM-DD') >= CURRENT_DATE - interval '30 days'
    GROUP BY 
        asin,
...
CREATE TEMP TABLE unified_daily_metrics AS (
    ...
    FROM deal_orders udm
        ...
        -- AND udm.customer_shipment_item_id = fp.customer_shipment_item_id
);

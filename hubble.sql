...
CREATE TEMP TABLE order_metrics AS (
    SELECT
        o.*,
        TO_DATE(LAG(o.order_date) OVER (PARTITION BY maa.brand_code, o.customer_id ORDER BY o.order_date),'YYYY-MM-DD') AS last_purchase_date,
        LAG(o.ASIN) OVER (PARTITION BY maa.brand_code, o.customer_id ORDER BY o.order_date) AS last_purchase_asin,
        -- cp.is_sns,
        ...
        cp.revenue_share_amt as revenue_share_amt,
        cp.display_ads_amt as display_ads_amt,
        cp.subscription_revenue_amt as subscription_revenue_amt
    FROM orders o
...
CREATE TEMP TABLE array_data as (
    select asin, split_to_array(purchase_order_ids , ',') as order
...
CREATE TEMP TABLE asin_promos as (
    select t.asin, orders as promo_purchase_id
